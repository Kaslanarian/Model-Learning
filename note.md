...
\begin{bmatrix}
    ...
    \vdots&\ddots&\vdots\\
    x_{n1}&\cdots&x_{np}\\
\end{bmatrix}=(x_1,x_2,\cdots,x_p)
$$
我们希望有这样的新指标$Z_{m}$:
$$
(z_1,z_2,\cdots,z_m)^\top=L_{m\times p}\times(x_1,x_2,\cdots,x_p)^\top
$$
...
2. $z_1$是$x_i$的一切线性组合中方差最大者；

3. $z_2$是与$z_1$不想关的$x_1,x_2,...$的所有线性组合中方差最大者；

4. 以此类推；

5. 新指标$z_i$分别成为原指标的第$i$主成分

### 步骤

1. 对原始数据标准化：$x\to X(X_{ij}=\dfrac{x_{ij}-\bar{x_j}}{S_j})$；

2. 计算$X$的协方差矩阵$R$（两步可合成为一步：直接计算$x$的样本系数矩阵）；

3. 计算$R$的特征值和特征向量，从大到小排列；

4. 计算主成分贡献率和累计贡献率：
$$
贡献率=\dfrac{\lambda_i}{\sum_{k=1}^p\lambda_k}(i=1,2,\cdots,p)\\
累计贡献率=\dfrac{\sum_{k=1}^i\lambda_k}{\sum_{k=1}^p\lambda_k}
$$

5. 取累计贡献率超过$80\%$的特征值所对应的第一，二，...，$m$个主成分；

6. 第$i$个主成分:
$$
F_i=a_{1i}X_1+a_{2i}X_2+\cdots+a_{pi}X_p=\bold{a}_i\bold X(\bold a_i为\lambda_i对应的特征向量)
$$

7. 分析主成分意义：对某个主成分而言，指标前面的系数越大，代表该指标对该主成分影响越大

8. 进行后续分析：用于聚类分析(方便画图)，用于回归分析